...
const vote = (
  ...
  votes = votes.reduce(
    (acc: number[], curr) => [...acc, Math.floor(curr % possibleOutcomes)],
    []
  );
...
const getBeat = (stateRoot: string) => {
  ...
  return {
    path: `${firstVote}/2/${secondVote}.mp3`,
    firstVote
  };
};

const getOtherPaths = (firstVote: number, txnHash: string) =>
  [0, 1, 4].map((category) => [
      ...
      return Promise.all(
        otherPaths.map((paths) =>
          crunker
            ...
            .catch((error: any) =>
              console.error(
                `GET ${error.url} ${error.status} (${error.statusText})`
              )
            )
        )
      )
        .then((buffers: (void | AudioBuffer)[]) =>
          crunker.concatAudio(buffers as AudioBuffer[])
        )
        .then((buffer: AudioBuffer) =>
          crunker
            .fetchAudio(...beatPaths)
            .then((buffers) => crunker.concatAudio(buffers))
            .then((second_buffer: AudioBuffer) =>
